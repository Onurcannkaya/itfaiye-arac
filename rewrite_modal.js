...
const endMarker = '{/* Şifre Sıfırlama Bölümü */}';
...
// Find the end of the Şifre Sıfırlama Bölümü. It ends at </DialogContent> basically.
const endOfModalIndex = content.indexOf('</DialogContent>', startIndex);
// We want to replace everything from startIndex up to the `</div>` before `)}` and `<DialogFooter`
// Let's find the DialogFooter
const footerIndex = content.indexOf('<DialogFooter', startIndex);
if (footerIndex === -1) { console.error('Footer not found'); process.exit(1); }

// We will replace from startIndex to footerIndex.
// But wait, the inner div closes right before DialogFooter. Let's find the matching `</div>\n            </div>\n          )}`
const endSliceIndex = content.lastIndexOf('</div>', footerIndex);
const prevEndSliceIndex = content.lastIndexOf('</div>', endSliceIndex - 1);
...
const tail = content.substring(prevEndSliceIndex);
