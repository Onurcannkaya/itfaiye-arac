...
export default function BakimPage() {
  ...
  const totalFuelCost = fuelLogs.reduce((s, f) => s + f.tutar, 0)
  const totalMaintenanceCost = maintenanceLogs.reduce((s, m) => s + (m.maliyet || 0), 0)
        ...
        <Card>
          <CardContent className="p-4 flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{fuelLogs.length}</p>
            </div>
          </CardContent>
        </Card>
        <Card>
