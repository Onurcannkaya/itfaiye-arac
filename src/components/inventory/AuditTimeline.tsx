"use client"
import { mockAuditLogs, type AuditLogEntry } from "@/lib/data"
import { COMPARTMENT_NAMES } from "@/lib/constants"
import { Clock, User, CheckCircle2, AlertCircle, AlertTriangle, ChevronDown, ChevronUp } from "lucide-react"
...
export function AuditTimeline({ plaka, compartmentKey }: AuditTimelineProps) {
  ...
  const logs = mockAuditLogs.filter(
    (log) => log.plaka === plaka && log.compartmentKey === compartmentKey
  )
                  ...
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      ...
                      <div className="flex items-center gap-2 mt-1 text-[11px] text-muted-foreground">
                        ...
                        <span className="font-medium">{formatRelativeTime(log.checkedAt)}</span>
                      </div>
                    </div>
